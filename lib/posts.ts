...
export const posts: Post[] = [
  /* ─────────────── WITHY — 넷플릭스 워치파티 크롬 익스텐션 ─────────────── */
  {
    ...
    project: "withy",
    theories: [
      { id: "isolation", role: "원인 개념" },
      { id: "propagation", role: "해결 기법" },
    ],
    ...
    sections: [
      ...
      {
        heading: "덤으로 — 채팅 치면 영상이 멈추던 문제",
        tone: "trouble",
        body: "채팅창에 Space를 치면 영상이 일시정지됐다. 넷플릭스가 가로챈 게 아니라, 키 이벤트가 조상 라인을 타고 document까지 올라가고 거기 넷플릭스 단축키 리스너가 있을 뿐이다. 남의 페이지에 UI를 얹었기 때문에 생긴 문제다. window에 capture 단계로 리스너를 걸어 넷플릭스보다 먼저 잡고, 우리 UI에 포커스가 있으면서 단축키에 해당하는 키일 때만 stopPropagation으로 전파를 끊었다. preventDefault는 하지 않는다 — 글자는 찍혀야 하니까. 차단이 아니라 통과가 기본이다.",
      },
      {
        heading: "결과와 한계",
        tone: "theory",
  ...
  },
  {
    ...
    readMinutes: 7,
    project: "withy",
    theories: [{ id: "realtime", role: "해결 기법" }],
  ...
  /* ─────────────── Petfolio — 반려동물 공동 가계부 ─────────────── */
  {
    ...
    sections: [
      ...
      },
      {
        heading: "적용 범위 — 정직하게",
        tone: "theory",
        body: "스키마 검증은 설계와 일부 화면 적용까지 갔고, 전면 롤아웃 전에 프로젝트가 끝났다. 다만 구조는 남았다: 백엔드가 변경되어도 프론트엔드가 스스로 방어하는 형태, 그리고 '외부에서 온 데이터는 경계에서 검증한다'는 원칙.",
      },
    ],
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        tone: "theory",
        body: "ApiError·NetworkError·AuthError·ParseError로 에러 타입 계층을 Tagged Error로 규격화하고, Effect-TS의 에러 채널에 실었다. Effect<A, ApiError | NetworkError | ...> 형태라 함수 시그니처에 '무엇이 실패할 수 있는가'가 드러나고, 분기를 하나라도 빠뜨리면 컴파일 에러가 난다. 런타임에 가서야 알던 것을 컴파일 단계로 당긴 것이다.",
        code: "// 분기 누락 = 컴파일 에러\nEffect.catchTags(program, {\n  ApiError:     (e) => showBanner(e.message),\n  NetworkError: ()  => showRetry(),      // 재시도 의미 있음\n  AuthError:    ()  => redirectLogin(),\n  ParseError:   ()  => showBanner(\"...\"), // 재시도 버튼 없음\n});",
      },
      {
      ...
      {
        heading: "결과 — 에러 종류마다 다른 UX",
        tone: "theory",
        body: "NetworkError에는 재시도를, AuthError에는 재로그인을, ParseError에는 재시도 없는 안내를 준다. 놓친 분기가 크래시가 되는 대신, 모든 실패가 각자 정해진 화면을 갖게 됐다.",
      },
    ],
  },
  ...
  /* ─────────────── Tickle — 공정한 티켓 예매 플랫폼 ─────────────── */
  {
    ...
    readMinutes: 7,
    project: "tickle",
    theories: [{ id: "repaint", role: "관련 개념" }],
    ...
    sections: [
      ...
      {
        heading: "해결 — 좌석 156개를 Canvas 하나로",
        tone: "theory",
        body: "좌석 맵 전체가 <canvas> 태그 단 하나다. 좌석 배치를 순회해 사각형 정보(id·x·y·크기) 배열을 useMemo로 한 번 만들고, 그리기는 React를 모르는 순수 함수로 분리했다. Canvas는 한 번 그리면 픽셀만 남고 '여기가 A열 3번'이라는 정보가 사라진다 — 사각형 배열이 유일한 진실이고, 화면은 그걸 시각화한 결과일 뿐이다. 클릭은 좌표를 받아 어느 사각형 안인지 역산하는 히트 테스트로 매핑했다.",
      },
      {
  ...
  },
  {
    ...
    readMinutes: 5,
    project: "tickle",
    theories: [{ id: "browser", role: "관련 개념" }],
  ...
  },
  {
    ...
    project: "tickle",
    theories: [
      { id: "eventloop", role: "원인 개념" },
      { id: "hooks", role: "해결 기법" },
      { id: "render", role: "원인 개념" },
    ],
