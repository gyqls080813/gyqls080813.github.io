...
export const entryIdeas: Idea[] = [
  {
    id: "craft-url",
    name: "URL 설계",
    tagline: "이 정보를 URL의 어디에 담을 것인가, 아니면 담지 않을 것인가",
    intro:
    ...
    sections: [
      {
        ...
        blocks: [
          {
            label: "질문은 하나다",
            body: "이 정보를 URL의 어디에 담을 것인가, 아니면 URL에 안 담을 것인가. 담을 자리는 셋(path, query, hash)이고 안 담는 선택이 하나 더 있어 선택지는 넷입니다. 넷 중 무엇을 고르든 판단의 축은 같습니다 — 이 값이 공유와 새로고침에 살아남아야 하는가, 살아남아야 한다면 그것은 '무엇을 보는지'인가 '어떻게 보는지'인가.",
          },
        ],
      },
      {
        ...
        blocks: [
          ...
          {
            label: "URL에 안 둔다 — 그 순간만 필요한 것",
            body: "모달이 열렸는지, 어디에 마우스가 올라가 있는지, 입력 중인 글자. 공유하거나 새로고침했을 때 되살아나지 않아도 되는 값은 컴포넌트 state에 둡니다. 가장 단순하고 URL을 더럽히지 않습니다. 대신 다른 탭이나 새로고침에는 남지 않습니다.",
          },
        ],
      },
      {
        ...
        blocks: [
          {
            label: "판별 순서",
            body: "먼저 '공유와 새로고침에 살아남아야 하나'를 묻습니다. 아니면 state입니다. 살아남아야 하면 '없으면 404가 맞나'를 묻습니다. 맞으면 path, 기본값이 있어도 되면 query입니다. 마지막으로 '서버는 몰라도 되는 순수한 위치값인가'면 hash입니다. 이 순서로 물으면 한 값이 두 자리에 동시에 해당하는 일이 없습니다.",
